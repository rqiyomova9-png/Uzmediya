...
BOT_FLOOD_MAX     = 8     # botdan 5 soniyada 8 tadan ko'p xabar = kamaytir
...
async def save_now() -> bool:
    """
    DARHOL saqlash — lokal faylga sinxron, JSONBlob ga fon rejimida.
    Bot javob berishni kutmaydi — sekinlashmaydi.
    """
    global _jsonblob_timer_task
    if _jsonblob_timer_task and not _jsonblob_timer_task.done():
        _jsonblob_timer_task.cancel()
    try:
        ...
        logger.error(f"Lokal saqlash xato: {e}")
    # JSONBlob ga fon rejimida yozamiz — bot kutmaydi
...
SUB_CACHE_TTL = 60  # 60 soniya — Telegram API yukini kamaytiradi
...
def movie_episodes_kb(movie: dict, code: str, user_id, page: int = 0):
    ...
    # "Barcha tekin qismlarni ko'rish" tugmasi — faqat bepul qismlar mavjud bo'lsa
    free_eps = [
        str(i + 1) for i in range(total)
        if price_to_int(prices.get(str(i + 1))) == 0
    ]
    if free_eps:
        rows.append([ibtn(
            _B(f"🎬 Barcha tekin qismlarni ko'rish ({len(free_eps)} ta)"),
            data=f"watch_all_free|{code}",
            style="success"
        )])
    return ikb(rows)
    ...
    try:
        ...
        if msg_id:
            ...
            try:
                ...
                logger.debug(f"auto_post edit xato ({code}): {e}")
...
_FFMPEG_AVAILABLE = None  # None = tekshirilmagan, True/False = natija

def _check_ffmpeg() -> bool:
    """ffmpeg mavjudligini bir marta tekshiradi va keshlaydi."""
    ...
    try:
        ...
        pass
    logger.info("ℹ️ ffmpeg topilmadi — watermarksiz ishlaydi")
...
async def sv_watermarked(bot, chat_id: int, file_id: str, caption: str,
    ...
    """
    # Bir marta tekshiriladi va keshlangan natija ishlatiladi
    if not _check_ffmpeg():
        logger.debug("ffmpeg yo'q — watermarksiz yuborilmoqda")
    ...
    try:
        async def _do_watermark():
            ...
            if success and os.path.getsize(tmp_out) > 1000:
                ...
                logger.debug("Watermark: ffmpeg muvaffaqiyatsiz — originalni yuboramiz")
                return await sv(bot, chat_id, file_id, caption, markup, pm, protect)
        ...
        logger.debug(f"sv_watermarked: 45s timeout — originalni yuboramiz")
        try:
            return await sv(bot, chat_id, file_id, caption, markup, pm, protect)
...
async def check_subscription(user_id, bot) -> list:
    ...
    async def check_one(ch):
        try:
            ...
            if not chat_ref:
                return None
            # 5 soniya timeout — Telegram sekin bo'lsa bot qotib qolmasin
            member = await asyncio.wait_for(
                bot.get_chat_member(chat_ref, user_id), timeout=5.0
            )
            ...
            logger.debug(f"Sub check timeout: {ch.get('username','?')}")
            return None  # Timeout — obuna bo'lgan deb hisoblaymiz (bot bloklamasin)
        except Exception as e:
            logger.debug(f"Sub check {ch.get('username','?')}: {e}")
            return None

    try:
        results = await asyncio.wait_for(
            asyncio.gather(*[check_one(ch) for ch in channels], return_exceptions=True),
            timeout=8.0
        )
    except asyncio.TimeoutError:
        logger.debug("check_subscription umumiy timeout — bo'sh ro'yxat")
        _sub_cache_set(user_id, [])
        return []
...
async def send_movie_menu(src, context, code: str):
    ...
    try:
        ...
        logger.warning(f"send_movie_menu xato: {e}")
...
async def _send_kino_list_page(bot, chat_id: int, page: int = 0, query=None):
    ...
    if PIL_AVAILABLE:
        try:
            ...
            logger.debug(f"kino_list surat xato: {e}")
    ...
    if query is not None and img_buf is not None:
        try:
            ...
            logger.debug(f"kino_list edit_media xato (fallback yangi xabar): {e}")
            try: img_buf.seek(0)
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        logger.debug(f"start inline kb xato: {_e}")

    # Reply menyu — alohida try/except (birinchi xabar muvaffaqiyatli bo'lmasa ham)
    try:
        await sm(context.bot, user.id, "👇 Menyu",
                 main_menu_kb(is_admin=(is_any_admin(user.id))))
    except Exception as _e:
        logger.debug(f"start menyu xato: {_e}")
...
async def callback_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # ── Darhol Telegram'ga javob beramiz (10s timeout bor) ──
    # show_alert=True ishlatadigan funksiyalar o'zlari answer chaqiradi
    # Bu yerda faqat "bosildi" belgisi sifatida javob beramiz
    try:
        await q.answer()
    except Exception:
        pass  # Allaqachon javob berilgan yoki timeout — davom etamiz
        ...
        try:
            ...
            try:
                ...
                if "not modified" not in err_str and "message_not_modified" not in err_str:
                    logger.debug(f"adm_perm edit kb: {e}")
        except Exception as e:
    ...
    if data.startswith("watch_all_free|"):
        await cb_watch_all_free(update, context)
        return
    ...
    if data.startswith("cc_check|"):
        ...
        if cc_status == "paid":
            ...
            try:
                ...
                logger.debug(f"cc_check admin notify xato: {_e}")
    ...
    if price > 0 and not is_episode_paid(user_id, code, ep):
        ...
        if balance >= price:
            ...
            try:
                ...
                logger.debug(f"Admin balance notify xato: {_e}")
...
async def cb_buy_all(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        logger.debug(f"Admin buy_all notify xato: {_e}")
...
# ══════════════════════════════════════════════════════════
# CALLBACK: BARCHA TEKIN QISMLARNI KO'RISH
# ══════════════════════════════════════════════════════════

async def cb_watch_all_free(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """
    Foydalanuvchi 'Barcha tekin qismlarni ko'rish' tugmasini bosadi.
    Kinodagi barcha bepul (narxi 0 yoki belgilanmagan) qismlarni ketma-ket yuboradi.
    """
    q = update.callback_query
    code = q.data.split("|", 1)[1].upper()
    user_id = q.from_user.id
    movie = RAM.movies.get(code)
    if not movie:
        await sm(context.bot, user_id, "❌ Kino topilmadi.")
        return

    eps = movie.get("episodes", [])
    prices = movie.get("prices", {}) or {}
    free_eps = [
        str(i + 1) for i in range(len(eps))
        if price_to_int(prices.get(str(i + 1))) == 0
    ]

    if not free_eps:
        await sm(context.bot, user_id, "❌ Bu kinoda tekin qismlar mavjud emas.")
        return

    bot_me = await context.bot.get_me()
    share_url = f"https://t.me/share/url?url=https://t.me/{bot_me.username}?start=code_{code}"

    await sm(context.bot, user_id,
             f"🎬 <b>{movie.get('title')}</b>\n"
             f"📺 Jami <b>{len(free_eps)} ta</b> tekin qism yuborilmoqda...")
    ...
    for ep in free_eps:
        ...
        try:
            await sv_watermarked(
                context.bot, user_id, eps[idx], caption,
                user_id=user_id,
                markup=share_kb(share_url), protect=True
            )
        except Exception as e:
            logger.error(f"watch_all_free — {ep}-qism yuborishda xato: {e}")
        # Flood oldini olish uchun kichik pauza
    ...
    async def update_stats():
        try:
            movie.setdefault("views", {})
            uid_str = str(user_id)
            RAM.ensure_user(uid_str)
            for ep in free_eps:
                movie["views"][ep] = movie["views"].get(ep, 0) + 1
                RAM.users[uid_str]["watched"][f"{code}_{ep}"] = True
                RAM.stats["total_views"] = RAM.stats.get("total_views", 0) + 1
            await schedule_save()
        except Exception as e:
            logger.error(f"watch_all_free update_stats xato: {e}")
    asyncio.create_task(update_stats())
...
async def cb_topup_payment(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if action == "topup_no":
        ...
        try:
            ...
            logger.debug(f"topup_no edit xato: {e}")
    ...
    try:
        ...
        logger.debug(f"topup edit xato: {e}")
...
async def admin_state_handler(update, context, text: str) -> bool:
    ...
    if state == "add_admin_id":
        ...
        if not already_admin:
            try:
                ...
                logger.debug(f"Yangi admin notify xato ({target}): {e}")
        return True
    ...
    if state == "add_soruvli_kanal":
        ...
        if username:
            try:
                ...
                logger.debug(f"So'rovli kanal get_chat xato: {e}")
        ...
        if invite_link:
            try:
                ...
                logger.debug(f"So'rovli kanal invite get_chat xato: {e}")
    ...
    if state == "premium_days":
        ...
        try:
            ...
            logger.debug(f"premium notify {target}: {e}")
        clear_admin_state(context)
    ...
    if state == "qism_och_price":
        ...
        try:
            ...
            logger.debug(f"qism_och notify xato: {e}")
...
async def join_request_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        try:
            ...
            logger.debug(f"Join request tasdiqda xato {user.id} → {chat_id}: {e}")
            return
...
def main():
    ...
    async def _startup_notify(context_job):
        try:
            ...
            try:
                def _get_db_user_count():
                    ...
                    try:
                        with conn.cursor() as cur:
                            cur.execute(
                                "SELECT jsonb_object_keys(value->'users') "
            ...
            logger.debug(f"Startup notify xato: {e}")
