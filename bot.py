...
DEFAULT_BTN = {
    ...
    "kanal_qosh":     _B('Ommaviy kanal havolasi'),
    "kanal_uch":      _B('Kanal ochirish'),
    "kanal_royxat":   _B('Kanallar royxati'),
    "oddiy_havola":   _B("Oddiy havola"),
    "soruvli_kanal":  _B("So'rovli havola"),
...
# ── MAJBURIY KANAL TUGMASINI ANIQLASH (bulletproof) ─────────
# Bold Unicode (Mathematical Sans-Serif Bold) belgilarini oddiy ASCII ga
# qaytaradi, emoji/whitespace ni tozalaydi va katta/kichik harfga bog'liq emas.
def _normalize_btn_text(text: str) -> str:
    if not text:
        return ""
    # Emoji prefix va boshqa noziq emoji-larni olib tashlaymiz
    t = re.sub(
        r'[\U0001F000-\U0001FFFF\u2600-\u27BF\uFE00-\uFE0F\u200d\ufe0f]+',
        ' ', text
    )
    # Bold sans-serif uppercase 𝗔-𝗭 (U+1D5D4..U+1D5ED) → A-Z
    # Bold sans-serif lowercase 𝗮-𝘇 (U+1D5EE..U+1D607) → a-z
    # Bold digits 𝟬-𝟵 (U+1D7EC..U+1D7F5) → 0-9
    out = []
    for ch in t:
        cp = ord(ch)
        if 0x1D5D4 <= cp <= 0x1D5ED:
            out.append(chr(cp - 0x1D5D4 + ord('A')))
        elif 0x1D5EE <= cp <= 0x1D607:
            out.append(chr(cp - 0x1D5EE + ord('a')))
        elif 0x1D7EC <= cp <= 0x1D7F5:
            out.append(chr(cp - 0x1D7EC + ord('0')))
        else:
            out.append(ch)
    norm = "".join(out)
    # Hamma whitespace ni bitta bo'shliqqa keltiramiz, lower qilamiz
    norm = re.sub(r'\s+', ' ', norm).strip().lower()
    return norm


def is_maj_kanal_text(text: str) -> bool:
    """"Majburiy kanal" tugmasi bosilganini har qanday format/emoji/bold da aniqlaydi."""
    n = _normalize_btn_text(text)
    if not n:
        return False
    # Aniq moslik yoki ikkala kalit so'z mavjud bo'lsa
    if n == "majburiy kanal":
        return True
    if "majburiy" in n and "kanal" in n:
        return True
    return False
...
async def text_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if is_any_admin(uid):
        ...
        # Bulletproof: bold-unicode, emoji prefix, whitespace farqlariga qaramay aniqlaymiz.
        if is_maj_kanal_text(text):
            logger.info(f"[MAJ_KANAL] uid={uid} tugma bosildi → menyu ochilmoqda")
            try:
                clear_admin_state(context)
            except Exception as _e:
                logger.warning(f"[MAJ_KANAL] clear_admin_state xato: {_e}")
                # Hech bo'lmaganda asosiy bayroqlarni tozalaymiz
                for _k in ("admin_state", "emoji_menu", "editing_btn_key",
                           "bc_adding_btn", "reply_to", "awaiting_check"):
            ...
            try:
                ...
                logger.error(f"[MAJ_KANAL] menyu yuborishda xato: {e}")
                try:
                    await context.bot.send_message(
                        uid,
                        "📡 <b>Majburiy kanal boshqaruvi</b>\n\nMenyu ochildi.",
                        parse_mode="HTML"
                    )
                except Exception:
                    pass
        ...
        if context.user_data.get("channel_manage_menu"):
            ...
            # Ommaviy kanal havolasi (a'zolik tekshiriladi)
            if text_p == strip_emoji_prefix(bt("kanal_qosh")).strip().lower():
                context.user_data["admin_state"] = "add_channel_username"
                await sm(context.bot, uid,
                    "📢 <b>Ommaviy kanal havolasi qo'shish</b>\n\n"
                    "Bu turdagi kanal a'zoligi <b>avtomatik tekshiriladi</b>.\n"
            ...
            # Oddiy havola (har qanday URL — tekshirilmaydi)
            if text_p == strip_emoji_prefix(bt("oddiy_havola")).strip().lower():
                ...
                await sm(context.bot, uid,
                    ...
                    "Bu turdagi havola foydalanuvchiga shunchaki ko'rsatiladi (a'zolik tekshirilmaydi).\n\n"
                    "Havola nomini kiriting (masalan: <code>Kino kanali</code>):")
                return

            # So'rovli havola (join request)
            if text_p == strip_emoji_prefix(bt("soruvli_kanal")).strip().lower():
                context.user_data["admin_state"] = "add_soruvli_kanal"
                await sm(context.bot, uid,
                    "📨 <b>So'rovli havola qo'shish</b>\n\n"
                    "Foydalanuvchi tugma orqali a'zolikka <b>so'rov yuboradi</b>, bot avtomatik tasdiqlaydi.\n"
                    "⚠️ Bot kanalga <b>admin</b> bo'lib, <b>\"A'zolikni boshqarish\"</b> huquqi bo'lishi shart!\n\n"
...
async def admin_buttons(update, context, text: str):
    ...
    if _btn_match("maj_kanal") or is_maj_kanal_text(text):
        logger.info(f"[MAJ_KANAL/admin_buttons] uid={uid} → menyu ochilmoqda")
        context.user_data.pop("admin_state", None)
        ...
        try:
            ...
            logger.error(f"[MAJ_KANAL/admin_buttons] menyu yuborishda xato: {e}")
            await context.bot.send_message(
                uid,
                "📡 <b>Majburiy kanal boshqaruvi</b>",
                parse_mode="HTML"
